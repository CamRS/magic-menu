...
export default function HomePage() {
  ...
  const refreshDropboxToken = async () => {
    try {
      const authHeader = Buffer.from(`${import.meta.env.VITE_DROPBOX_APP_KEY}:${import.meta.env.VITE_DROPBOX_APP_SECRET}`).toString('base64');
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const dbx = new Dropbox({ accessToken: newAccessToken });
      const reader = new FileReader();
